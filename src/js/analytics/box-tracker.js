...
export default function boxTracker(
  elementSelector, // class of element we are tracking
    ...
    if (!boxState[tKey].triggered) {
      if (isScrolledIntoView(elem)) {
        if (!boxState[tKey].inView) {
          if (btDebugging) console.log(`${label} is in view`);
          boxState[tKey].inView = true;
          // start timing
          boxState[tKey].lh = setTimeout(() => {
            if (btDebugging) console.log(`${label} chart-in-view triggered`);
            boxState[tKey].triggered = true;
            const tevent = new window.CustomEvent(eventName, {
              detail: { label, elem: elementSelector },
            });
            window.dispatchEvent(tevent);
          }, minimumViewMS);
        }
      } else if (boxState[tKey].inView) {
        if (btDebugging) console.log(`${label} is out of view`);
